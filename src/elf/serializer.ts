	...
	
	{
		...
		switch (dataType) {
			...
			case DataType.DataUi: {
				...
				symbolSizeOverrides.set(`wld::fld::data::s_UIIconData`, (binary.data.iconEntry.length + 1) * FILE_TYPES[DataType.UiIconMenu].size)
				serializeObjects(data, DataType.UiIconMenu, binary.data.iconEntry, { padding: 1 })

				
				// mail map
				symbolLocationReference.set(`wld::fld::data::s_UIMailData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIMailData`, (binary.data.mailEntry.length + 1) * FILE_TYPES[DataType.UiMail].size)
				serializeObjects(data, DataType.UiMail, binary.data.mailEntry, { padding: 1 })

				// sea map
				symbolLocationReference.set(`wld::fld::data::s_UIMapData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIMapData`, (binary.data.seaEntry.length + 1) * FILE_TYPES[DataType.UiSeaMap].size)
				...
				symbolSizeOverrides.set(`wld::fld::data::s_UIUranaisiNextData`, binary.data.uranaisi.length * FILE_TYPES[DataType.UiUranaisi].size)
				serializeObjects(data, DataType.UiUranaisi, binary.data.uranaisi)
				
				// star
				symbolLocationReference.set(`wld::fld::data::s_UIStarpieceData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIStarpieceData`, binary.data.starpiece.length * FILE_TYPES[DataType.UiStar].size)
				serializeObjects(data, DataType.UiStar, binary.data.starpiece)
				
				// shine
				symbolLocationReference.set(`wld::fld::data::s_UIShineData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIShineData`, binary.data.shine.length
					* FILE_TYPES[DataType.UiShine].size)
				serializeObjects(data, DataType.UiShine, binary.data.shine)
				
				// art
				symbolLocationReference.set(`wld::fld::data::s_UIGalleryArtData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIGalleryArtData`, binary.data.galleryart.length * FILE_TYPES[DataType.UiGalleryArt].size)
				serializeObjects(data, DataType.UiGalleryArt, binary.data.galleryart)

				// sound
				symbolLocationReference.set(`wld::fld::data::s_UIGallerySoundData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIGallerySoundData`, binary.data.gallerysound.length * FILE_TYPES[DataType.UiGallerySound].size)
				serializeObjects(data, DataType.UiGallerySound, binary.data.gallerysound)		
				
				// ac master
				symbolLocationReference.set(`wld::fld::data::s_UIAcMasterData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UIAcMasterData`, binary.data.acmaster.length * FILE_TYPES[DataType.UiAcMaster].size)
				serializeObjects(data, DataType.UiAcMaster, binary.data.acmaster)						

				// select window
				symbolLocationReference.set(`wld::fld::data::s_UISelectWindowData`, new Pointer(dataWriter.size))
				symbolSizeOverrides.set(`wld::fld::data::s_UISelectWindowData`, binary.data.selectwindow.length * FILE_TYPES[DataType.UiSelectwindow].size)
				serializeObjects(data, DataType.UiSelectwindow, binary.data.selectwindow)						
				...
				rodataWriter.writeInt32(binary.data.model.length)
				rodataWriter.writeInt32(binary.data.modelProperty.length)
				rodataWriter.writeInt32(binary.data.msg.length)
				rodataWriter.writeInt32(binary.data.style.length)
				rodataWriter.writeInt32(binary.data.iconEntry.length)
				rodataWriter.writeInt32(binary.data.mailEntry.length)
				rodataWriter.writeInt32(binary.data.seaEntry.length)
				rodataWriter.writeInt32(binary.data.uranaisi.length)
				rodataWriter.writeInt32(binary.data.starpiece.length)
				rodataWriter.writeInt32(binary.data.shine.length)
				rodataWriter.writeInt32(binary.data.galleryart.length)
				rodataWriter.writeInt32(binary.data.gallerysound.length)
				rodataWriter.writeInt32(binary.data.acmaster.length)
				rodataWriter.writeInt32(binary.data.selectwindow.length)
